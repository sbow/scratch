...
% https://github.com/AtsushiSakai/MATLABRobotics/blob/master/PathPlanning/PathSmoothing/PathSmoothingSample.m
...
[optPath,n]=PathSmoothing(path);
  
plot(path(:,1),path(:,2),'-.b');hold on;
plot(optPath(:,1),optPath(:,2),'-or');hold on;
axis([-1 7 -2 6])
legend('Before','After');
title(['Path Smoothing - n iterations: ',num2str(n)]);
grid on;

end

function [optPath,n]=PathSmoothing(path)
